...
public class MigrationApp {
    ...
    private static final String YOBI_SERVER = "http://yobi.navercorp.com/";
    ...
    private static String relativeLinksToAbsolutePath(String text){
        ...
        return text.replaceAll("(<img src=[\"\'])/(?<link>.*)([\"\']>)", "$1" + YOBI_SERVER + "$2$3")
            .replaceAll("\\[(?<text>[^\\]]*)\\]\\(/(?<link>[^\\)]*)\\)", "[$1](" + YOBI_SERVER + "$2)");
    }
    ...
    private static StringBuilder addAttachmentsString(@NotNull StringBuilder sb, ResourceType type, String id){
        try {
            ...
            for(Map<String, String> attachment: attachments){
                sb.append(String.format("\n[%s](%s)", attachment.get("name"), YOBI_SERVER + attachment.get("url")));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private static ObjectNode composePostJson(Posting posting) {
        String originalPostingLink = String.format("%s/%s/post/%s",
                YOBI_SERVER + posting.project.owner, posting.project.name, posting.getNumber());
        ...
        sb.append(addOriginalAuthorName(
                relativeLinksToAbsolutePath(posting.body), posting.authorLoginId, posting.authorName, "게시글", originalPostingLink));
    ...
    private static ObjectNode composeIssueJson(Issue issue) {
        String originalIssueLink = String.format("%s/%s/issue/%s",
                YOBI_SERVER + issue.project.owner, issue.project.name, issue.getNumber());
    ...
    private static List<ObjectNode> composeCommentsJson(AbstractPosting posting, String orgLink, ResourceType type) {
