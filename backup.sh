...
BACKUP_NEW="yobi.$DATE_NEW.tar"
BACKUP_OLD="yobi.$DATE_OLD.tar"
BACKUP_LOG=/data/CLI/logs/full_backup/$DATE_NEW.log

tar cvfh $BACKUP_PATH/$BACKUP_TYPE/$BACKUP_NEW $SRC_PATH/$SRC_TARGET >> $BACKUP_LOG
$NCFS_CMD put $BACKUP_PATH/$BACKUP_TYPE/$BACKUP_NEW ncfs://$NCFS_CONTAINER/$NCFS_PATH >> $BACKUP_LOG
$NCFS_CMD del ncfs://$NCFS_CONTAINER/$NCFS_PATH/$BACKUP_OLD >> $BACKUP_LOG
